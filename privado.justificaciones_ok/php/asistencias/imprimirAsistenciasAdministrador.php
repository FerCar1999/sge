...
// datos para cargar la lista 
$horaInicialClase =Database::getRow('select t.hora_inicial as inicio from horarios h, tiempos t where h.id_tiempo = t.id_tiempo and h.id_horario = ? ',array($_SESSION["id_horarioAdmin"]));


$hora_horario =Database::getRow('select ADDTIME(?,"0:10") as tiempo',array($horaInicialClase["inicio"]));

$id_personal = $_SESSION["id_maestro"];
$fecha = $_SESSION["ver_fecha_asistencia"];
...
where h.id_personal='.$id_personal.' and dia="'.$dia.'" and h.id_local=lo.id_local and h.id_seccion= sec.id_seccion and 
h.id_grado = gr.id_grado and h.id_asignatura=asig.id_asignatura and h.id_tiempo=ts.id_tiempo and h.id_especialidad=esp.id_especialidad and CURDATE() between h.inicio and h.fin and date("'.$fecha.'") between h.inicio and h.fin ';

if(isset($_SESSION["pk_horario"])){
	$sql.= 'and h.id_horario = '.$_SESSION["pk_horario"].' order by ts.hora_inicial asc limit 1';
}else {
	$sql.= 'and TIME_TO_SEC(("'.$hora_horario["tiempo"].'")) between TIME_TO_SEC(ts.hora_inicial) and TIME_TO_SEC(ts.hora_final) order by ts.hora_inicial asc limit 1';
}
...
foreach ($data as $row) {	
	...
	if($grupo ==="No aplica"){	
		...
		foreach ($alumnos as $estudiante) {
			...
			$guardados = Database::getRow(' select id_estudiante,contenido,observacion from asistencias where id_horario = ? and TIME_TO_SEC((fecha_hora)) between TIME_TO_SEC(?) and TIME_TO_SEC(?) and id_estudiante =? and DATE(fecha_hora) = date(?)',array($id_horario,$row['hinicio'],$row['final'],$id,$fecha));
			// obtiene si el alumno tiene lleda tarde
			$llegada_tarde = Database::getRow('select id_estudiante from impuntualidad where id_horario = ? and TIME_TO_SEC((fecha_hora)) between TIME_TO_SEC(?) and TIME_TO_SEC(?) and id_estudiante =? and DATE(fecha_hora) = date(?)',array($id_horario,$row['hinicio'],$row['final'],$id,$fecha));
			...
			// imprime la tabla
			$tabla.= "<tr><td>$codigo</td><td>$nombre</td><td><input  id='$id' id_horario='$id_horario' guardado='$atributo' class='access_check' type='checkbox' $checked> <label id='$id' for='$id'></td><td><input pk='$id'  id='$codigo' id_horario='$id_horario'  guardado='$atributo_tarde' class='access_check_tarde' type='checkbox' $checked_tarde $suspendido> <label id='$id' for='$codigo'></td>";
		}
	...
	if($tipo==="academico"){
		...
		foreach ($alumnos as $estudiante) {
			...
			$guardados = Database::getRow(' select id_estudiante,contenido,observacion from asistencias where id_horario = ? and id_estudiante =? and DATE(fecha_hora) = date(?) group by id_estudiante',array($id_horario,$id,$fecha));
			
			$llegada_tarde = Database::getRow('select id_estudiante from impuntualidad where id_horario = ?  and id_estudiante =? and DATE(fecha_hora) = date(?)',array($id_horario,$id,$fecha));
			...
			$justificado_bloque=Database::getRow("select count(*) as cantidad from bloques_justificados where fecha = date(?) and id_estudiante = ? and id_horario = ?",array($id,$id_horario,$fecha));
			$ausencia_previa=Database::getRow("select count(*) as cantidad from ausencias_justificadas where id_estudiante = ? and date(?) between inicio and fin",array($id,$fecha));
			$suspendidoData=Database::getRow("select s.id_suspendido from suspendidos s where s.id_estudiante=(select id_estudiante from estudiantes where codigo =?) and date(?) between s.inicio and s.fin",array($codigo,$fecha));
			...
			if($suspendido==="disabled"){
				$tabla.= "<tr><td>$numero_lista</td><td>$codigo</td><td>$nombre $estado_ausencia</td><td><input  type='checkbox' $checked> <label ></td><td><input pk='$id'  id='$codigo' id_horario='$id_horario'  guardado='$atributo_tarde' type='checkbox' $checked_tarde $suspendido> <label id='$id' for='$codigo'></td>";
			}
			else{
				$tabla.= "<tr><td>$numero_lista</td><td>$codigo</td><td>$nombre</td><td><input  id='$id' id_horario='$id_horario' guardado='$atributo' class='access_check' type='checkbox' $checked> <label id='$id' for='$id'></td><td><input pk='$id'  id='$codigo' id_horario='$id_horario'  guardado='$atributo_tarde' class='access_check_tarde' type='checkbox' $checked_tarde $suspendido> <label id='$id' for='$codigo'></td>";
			}
		}
	}
		...
		foreach ($alumnos as $estudiante) {
			...
			if($suspendido==="disabled"){
				$tabla.= "<tr><td>$numero_lista</td><td>$codigo</td><td>$nombre $estado_ausencia</td><td><input    guardado='$atributo'  type='checkbox' $checked $suspendido> <label id='$id' for='$id'></td><td><input  guardado='$atributo_tarde'  type='checkbox' $checked_tarde $suspendido> <label id='$id' for='$codigo'></td>";

			}else{
				$tabla.= "<tr><td>$numero_lista</td><td>$codigo</td><td>$nombre</td><td><input  id='$id' id_horario='$id_horario'  guardado='$atributo' class='access_check' type='checkbox' $checked $suspendido> <label id='$id' for='$id'></td><td><input pk='$id'  id='$codigo' id_horario='$id_horario'  guardado='$atributo_tarde' class='access_check_tarde' type='checkbox' $checked_tarde $suspendido> <label id='$id' for='$codigo'></td>";	
			}

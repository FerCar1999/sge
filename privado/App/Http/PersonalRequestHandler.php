...
$idOldPersonal = isset($_POST["idOldPersonal"]) ? trim(strip_tags($_POST["idOldPersonal"])): "";
$idNewPersonal = isset($_POST["idNewPersonal"]) ? trim(strip_tags($_POST["idNewPersonal"])): "";
...
// transferencia carga academica
else if($action == 4) {  
  $request = array(
    "idOldPersonal" => $idOldPersonal,
    "idNewPersonal" => $idNewPersonal
  );  
  $controller->transferAcademicLoad($request);  
}
?>
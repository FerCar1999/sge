...
if($id>0){
	$sql = "delete from inasistencias_totales  where id_inasistencia=?";
	Database::executeRow($sql, array($id));			
	try{
		$aditionalDescription = " Action Details: id_inasistencia = {$id}";
...
function addToBitacora($aditionalDescription){
	try {
		...
		BitacoraLogger::$description = "Inasistencia Total Eliminada";

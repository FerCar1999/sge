<?php 
    ...
    if(!isset($_SESSION['id_personal']) || $_GET['personal']==""){      
        header('Location: /404.php');
        exit();
    }
    $id_maestro = $_GET['personal'];
    ...
    // id recibidio por get
    $pk_id = $id_maestro;
    $bloque = 0;
    $diasPasados = 0;

    $horarioDocente = new horariosDocente();

    $horarioDocente->horarioDocente($pk_id);
    $tiempos = $horarioDocente->getTiempos();
    $lunes = $horarioDocente->getDia(1);
    $martes = $horarioDocente->getDia(2);
    $miercoles = $horarioDocente->getDia(3);
    $jueves = $horarioDocente->getDia(4);
    $viernes = $horarioDocente->getDia(5);
    $bloques = $horarioDocente->formatBloque($lunes,$martes,$miercoles,$jueves,$viernes);
...
'<html>
    ...
    <body>
        ...
        <div class="content">
            ...
            $horarioHtml .=  
            '<p style="text-align:center;font-weight:bold;padding-top:0mm;padding-left:10mm;font-size:11pt;">HORARIO 2019 - DOCENTE: '.$docente[0].'</p>';
            $horarioHtml .=  
            '<br>

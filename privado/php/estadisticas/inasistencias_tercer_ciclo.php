...
$periodos=Database::getRows("select nombre,fecha_inicial,fecha_final from etapas where id_nivel = 1",array());
...
foreach ($periodos as $row) {	
	$string_values = "";	
	$cantidades=Database::getRows("select count(*) as cantidad from inasistencias_totales it,estudiantes e, grados g where it.id_estudiante=e.id_estudiante and e.id_grado=g.id_grado and g.id_nivel = 1 and YEAR(it.fecha) = YEAR(CURDATE()) and it.fecha between ? and ?",array($row['fecha_inicial'],$row['fecha_final']));
...
$html = "<script>
function load_chart6(){
    Highcharts.chart('chart6', {

        title: {
            text: 'INASISTENCIAS TERCER CICLO'
        },

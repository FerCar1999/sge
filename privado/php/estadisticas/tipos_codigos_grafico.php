...
$tipos=Database::getRows("select id_tipo_codigo,nombre from tipos_codigos where estado = 'Activo'",array());
...
foreach ($tipos as $row) {	
	...
	$cantidades=Database::getRows("select count(d.id_disciplina) as cantidad,MONTH(d.fecha_hora) as mes from disciplina d,codigos c where  YEAR(d.fecha_hora) = YEAR(CURDATE()) and  d.id_codigo = c.id_codigo and c.id_tipo_codigo = ? GROUP BY  MONTH(d.fecha_hora) ",array($row['id_tipo_codigo']));
...
function load_chart3(){
    Highcharts.chart('chart3', {
        ...
        title: {
            text: 'CODIGOS ASIGNADOS POR CADA TIPO'
        },

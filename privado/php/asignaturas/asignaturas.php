	...
	if($id === null && !empty($nombre) && strlen($nombre)>0 && strlen($nombre) < 100){
		...
		// ejecuta la consulta
 		...
 		if($last_id>0){
			...
			try{
				...
				addToBitacora("Se ha agregado una nueva Asignatura",$aditionalDescription);
			}catch(Exception $e){}
		}
		else{
		...
		if(Database::executeRow($sql, $params)){
			...
			try{
				...
				addToBitacora("Se ha modificado una Asignatura",$aditionalDescription);
			}catch(Exception $e){}
		}
		else{
			echo 'existente';

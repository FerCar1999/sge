<?php 
	...
	$data=Database::getRows("select dc.id_disciplina as id,p.nombre as nombre,p.apellido as apellido,c.nombre as codigo,tc.nombre as tipo,date(dc.fecha_hora) as fecha, dc.observacion from disciplina dc, personal p, codigos c, tipos_codigos tc where dc.id_personal = p.id_personal and dc.id_codigo=c.id_codigo and c.id_tipo_codigo=tc.id_tipo_codigo and dc.id_estudiante=(select id_estudiante from estudiantes  where codigo=?) AND dc.fecha_hora BETWEEN DATE_SUB(NOW(), INTERVAL 1 HOUR) AND (SELECT NOW())",array($id));	
	foreach($data as $row)
	{
		$data_codigo = array();

<?php 
	...
	if($id_horario>0){
		$sql = 'delete from inasistencias_clases where id_horario = ?  and date(fecha_hora) = curdate() and id_estudiante =?';
	...
	if($clase["id_horario"]==null){
		...
		$sql_quitar_lista = "delete from inasistencias_clases where id_horario = ? and date(fecha_hora) = curdate() and id_estudiante =?";
		// parametros
			foreach($asistencia as $id_estudiante)
			{
				if($id_estudiante!=null){

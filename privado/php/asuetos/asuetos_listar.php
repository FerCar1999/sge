<?php
    ...
    $query = "select id_asueto,nombre,fecha_inicio,fecha_fin from asuetos where estado ='{$estado}' ";
    //consulta para crear la paginacion 
    $query_count ="select count(*) as total from asuetos where estado ='{$estado}'  ";
    ...
    $stmt->bind_result($id_asueto, $nombre, $fecha_inicio,$fecha_fin);
    
    // asigna el resultado a jsondataList
    while ($stmt->fetch()) {
        $asuetos = array();
        $asuetos["id"] = $id_asueto;
        $asuetos["nombre"] = $nombre;
        $asuetos["inicio"] = $fecha_inicio;
        $asuetos["fin"] = $fecha_fin;
        $jsondataList[]=$asuetos;
    }
    $stmt->close();
    // cierra la conexion 

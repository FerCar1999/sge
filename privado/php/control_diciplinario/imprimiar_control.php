...
$grado = Database::getRow('select id_especialidad, id_grado,id_grupo_tecnico from estudiantes where id_personal=? limit 1 ', array($_SESSION["id_personal"]));
 
 // obtiene el nivel del grado
$nivel = Database::getRow('select id_nivel from grados where id_grado = ?',array($grado['id_grado']));
$fechas = Database::getRow('select fecha_inicial,fecha_final from etapas where id_nivel=(select id_nivel from grados where id_grado=?) and CURDATE() between fecha_inicial and fecha_final',array($grado['id_grado']));
...
if(intval($nivel['id_nivel']) ==1){	
	$estudiantes = Database::getRows('select id_estudiante,fecha_procesado from estudiantes where procesado =1 and id_grado=? and id_especialidad =? and id_grupo_tecnico=?  and id_personal = ? order by apellidos desc',array($grado['id_grado'],$grado['id_especialidad'],$grado['id_grupo_tecnico'],$_SESSION["id_personal"]));	
}else {	
	$estudiantes = Database::getRows('select id_estudiante,fecha_procesado from estudiantes where procesado =1 and id_grado=? and id_especialidad =? and id_grupo_tecnico=? order by apellidos desc',array($grado['id_grado'],$grado['id_especialidad'],$grado['id_grupo_tecnico']));
}

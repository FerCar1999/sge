<?php
    ...
    $order_by = isset($_POST["orden"]) ? $_POST["orden"] : "c.nombre";
    ...
    $query = "SELECT c.id_codigo,c.nombre,c.descripcion,tp.nombre,tp.id_tipo_codigo FROM codigos c INNER JOIN tipos_codigos tp ON c.id_tipo_codigo = tp.id_tipo_codigo AND c.estado ='{$estado}'";
    //consulta para crear la paginacion 
    $query_count ="SELECT COUNT(*) as total FROM codigos c INNER JOIN tipos_codigos tp ON c.id_tipo_codigo = tp.id_tipo_codigo AND c.estado ='{$estado}'";
    //busquedas: like nombre,id etc...
    $campos_like="c.nombre";
